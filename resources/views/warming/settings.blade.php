@extends('layouts.app')
@section('title', 'الإعدادات — Email Warming')

@section('content')
<div class="space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-white">إعدادات الإرسال</h1>
            <p class="text-sm text-zinc-400 mt-1">تحكّم في طريقة عمل بوت الإرسال</p>
        </div>
    ...
    <form action="{{ route('warming.settings.update') }}" method="POST" class="space-y-6">
        ...
        {{-- Send Mode --}}
        <div class="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800/50 rounded-2xl p-6 space-y-5">
            ...
            <div class="grid gap-3">
                @foreach([
                    ['auto', '🤖 تلقائي بالكامل', 'البوت يملأ جميع الحقول ويضغط إرسال تلقائياً', 'emerald'],
                    ['manual_send', '✋ يدوي الإرسال فقط', 'البوت يملأ الحقول (المستلم + الموضوع + النص) لكن ينتظرك أنت تضغط Send', 'amber'],
                    ['full_manual', '🖐️ يدوي بالكامل', 'البوت يفتح صفحة Compose فقط — أنت تملأ وترسل بنفسك', 'red'],
                ] as [$value, $label, $desc, $color])
                <label class="flex items-start gap-4 p-4 rounded-xl border cursor-pointer transition-all
                    {{ $sendMode === $value ? "bg-{$color}-500/10 border-{$color}-500/30" : 'bg-zinc-800/30 border-zinc-700/50 hover:border-zinc-600/50' }}">
                    <input type="radio" name="send_mode" value="{{ $value }}" {{ $sendMode === $value ? 'checked' : '' }}
                           class="mt-1 accent-orange-500">
                    <div>
                        <p class="text-sm font-semibold text-white">{{ $label }}</p>
                        <p class="text-xs text-zinc-400 mt-0.5">{{ $desc }}</p>
                    </div>
                </label>
                @endforeach
            </div>
        ...
        <div class="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800/50 rounded-2xl p-6 space-y-4">
            ...
            <p class="text-xs text-zinc-400">شغّل أو أوقف بوت الإرسال من هنا. البوت سيفتح في نافذة Terminal جديدة.</p>
