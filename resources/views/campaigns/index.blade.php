@extends('layouts.app')
@section('title', 'الحملات التسويقية — ColdForge')
...
<div class="space-y-8">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-zinc-400 mt-1">أرسل إيميلات AI لقوائم العملاء المستهدفين</p>
        </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        @foreach([
            ['إجمالي الحملات', $stats['total'], 'text-violet-400'],
            ['يعمل الآن', $stats['running'], 'text-emerald-400'],
            ['مُجدولة', $stats['scheduled'], 'text-cyan-400'],
            ['مكتملة', $stats['completed'], 'text-green-400'],
        ] as [$label, $value, $colorClass])
        <div class="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800/50 rounded-xl p-4 text-center">
    ...
    <div class="space-y-3">
        ...
        <a href="{{ route('campaigns.show', $campaign) }}" class="block bg-zinc-900/50 backdrop-blur-sm border border-zinc-800/50 rounded-2xl p-5 hover:border-zinc-700/50 transition-colors group">
            <div class="flex items-center justify-between gap-4">
                <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-3">
                        <h3 class="text-sm font-semibold text-white group-hover:text-violet-300 transition-colors truncate">{{ $campaign->name }}</h3>
                        @php
                            $statusClasses = [
                    ...
                    <div class="flex items-center gap-4 mt-2 text-[11px] text-zinc-500">
                        @if($campaign->accounts && $campaign->accounts->count() > 0)
                            <span title="{{ $campaign->accounts->pluck('email')->join(', ') }}">📧 {{ $campaign->accounts->count() }} حسابات</span>
                        @else
                            <span>📧 —</span>
                        @endif
                        <span>👥 {{ $campaign->total_recipients }} مستلم</span>
                        <span>📅 {{ $campaign->send_start_date?->format('M d') ?? '—' }} · {{ $campaign->send_start_time }} → {{ $campaign->send_end_time }}</span>
                        <span>⏱ {{ $campaign->min_delay_minutes }}-{{ $campaign->max_delay_minutes }} دقيقة</span>
                        @if($campaign->followUps->count() > 0)
                            <span class="text-amber-400">🔄 {{ $campaign->followUps->count() }} Follow-Up</span>
                        @endif
                    </div>
                </div>
                <div class="text-right shrink-0">
                    <p class="text-lg font-bold {{ $progressColors[$campaign->status] ?? 'text-zinc-400' }}">{{ $campaign->progress_percent }}%</p>
                    <p class="text-[11px] text-zinc-500">{{ $campaign->sent_count }}/{{ $campaign->total_recipients }}</p>
                </div>
            </div>
            @if($campaign->total_recipients > 0)
            <div class="mt-3 w-full bg-zinc-800 rounded-full h-1.5">
                <div class="bg-gradient-to-r from-violet-500 to-purple-500 h-1.5 rounded-full transition-all" style="width: {{ $campaign->progress_percent }}%"></div>
            </div>
            @endif
        </a>
        @empty
        <div class="text-center py-16 bg-zinc-900/50 border border-zinc-800/50 rounded-2xl">
            ...
            <p class="text-zinc-400 font-medium mt-4">لم تنشئ أي حملة بعد</p>
            <p class="text-sm text-zinc-500 mt-1">أنشئ حملتك الأولى لبدء الإرسال التلقائي</p>
